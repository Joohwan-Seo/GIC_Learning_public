...
class WipingEnv(Env):
    def __init__(self, robot_name = 'fanuc', max_time = 15, show_viewer = False, obs_type = 'pos', testing = False, window_size = 1,
                 mixed_obs = False, plate_ori = 'arbitrary_x', use_ext_force = False, seed = None, fix_camera = False):
    ...
    def reset(self, angle_prefix = None):
        ...
        if not self.testing:
            ...
            rand_rpy = 2*(np.random.rand(3,) - 0.5) * 5 /180 * np.pi
...
if __name__ == "__main__":
    ...
    WE = WipingEnv(robot_name, show_viewer = True)
    WE.reset(angle_rad)
    WE.test()
