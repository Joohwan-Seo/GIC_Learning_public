...
class RobotEnvResidual(Env):
    def __init__(self, robot_name = 'fanuc', env_type = 'square_PIH', max_time = 15, show_viewer = False, 
                 obs_type = 'pos', hole_ori = 'default', testing = False, reward_version = None, window_size = 1, ECGIC = False, TCGIC = False):
        ...
        print('===================')
        print('I AM IN DEFAULT RESIDUAL ENV')
        print('===================')
        ...
        self.kd = 7
        self.ko = 10
        self.int_term = np.array([0,0]).reshape((-1,1))
        self.deriv_term = np.array([0,0]).reshape((-1,1))
    ...
    def get_expert_action(self):
        ...
        if self.ECGIC:
            ...
            R_xy = np.array([[0,1],[1,0]])
            a0 = self.kp * (x[1] - self.xd[1]) + self.Fe[0,0]
            a1 = self.kp * (x[0] - self.xd[0]) + self.Fe[1,0]
            ex = x[0] - self.xd[0]
            ey = x[1] - self.xd[1]

            e_xy = np.array([ex,ey]).reshape((-1,1))

            pid_action = self.get_pid_action(e_xy)

            a = R_xy @  pid_action + self.Fe[0:2,0]
            ...
            a3,a4,a5 = 0,0,0
            # a3, a4, a5 = self.Fe[3,0], self.Fe[4,0], self.Fe[5,0]

            action = np.array([a0,a1,a2,a3,a4,a5]) / self.max_force
    ...
    def impedance_control(self, action):
        ...
        f_force = self.convert_action(action)

        tau_tilde = -fg -Kd @ eV - f_force
    ...
    def get_pid_action(self,e_xy):
        self.int_term = self.int_term * 0.999 + self.dt * e_xy
        self.int_term = np.clip(self.int_term, -50, 50)

        if np.linalg.norm(e_xy) > 0.05:
            self.int_term = np.zeros((2,1))

        self.deriv_term = (e_xy - self.prev_exy)/self.dt

        pid_term = self.kp * e_xy + self.ki * self.int_term + self.kd * self.deriv_term
...
if __name__ == "__main__":
    ...
    env_type = 'square_PIH'
    show_viewer = True
    RE = RobotEnvResidual(robot_name, env_type, show_viewer = True, obs_type = 'pos', window_size = 1, hole_ori = 'default', ECGIC = False)
    RE.test()
