...
class RobotEnv(Env):
    ...
    def get_expert_action(self):
        ...
        if self.act_type == 'default':
            ...
            action += np.random.randn(3,) * np.array([.02, .02, .02])
            action = np.clip(action, -0.99, 0.99)
...
if __name__ == "__main__":
    ...
    env_type = 'square_PIH'
    show_viewer = True
    RE = RobotEnv(robot_name, env_type, show_viewer = True, obs_type = 'pos', window_size = 1, hole_ori = 'default', ECGIC = False, use_ext_force = False, testing = False, act_type = 'default')
    RE.test()
