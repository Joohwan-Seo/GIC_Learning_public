    ...
    for case in cases:
        ...
        if args.benchmark:
            env = RobotEnvBenchmark(show_viewer = args.vis, obs_type = args.obs_type, hole_ori = case, testing = True, 
                                    window_size = args.window_size, mixed_obs = args.mixed_obs, fix_camera = args.fix_camera)
        else:
            env = RobotEnv(show_viewer = args.vis, obs_type = args.obs_type, hole_ori = case, testing = True, 
                           window_size = args.window_size, use_ext_force=use_ext_force, mixed_obs = args.mixed_obs, fix_camera = args.fix_camera)
...
def test_policy_aug_random(args):
    ...
    if args.in_dist:
        if args.fix_seed:
            np.random.seed(1)
        angles = (np.random.rand(100) - 0.5) * np.pi
        simulation_time = args.H
    else:
        if args.fix_seed:
            np.random.seed(0)
        a = (np.random.rand(100) - 0.5)
        angles = np.sign(a) * (np.abs(a) * np.pi/2 + np.pi/2)
