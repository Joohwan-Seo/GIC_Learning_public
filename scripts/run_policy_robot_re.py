...
def test_policy(args):
    ...
    for case in cases:
        if args.benchmark:
            env = NormalizedBoxEnv(RobotEnvBenchmark(show_viewer = args.vis, obs_type = args.obs_type, hole_ori = case, testing = True, window_size = args.window_size, ECGIC = ECGIC))
        else:
            if args.residual:
                ...
                env = NormalizedBoxEnv(RobotEnv(show_viewer = args.vis, obs_type = args.obs_type, hole_ori = case, testing = True, window_size = args.window_size, ECGIC = ECGIC, TCGIC = TCGIC, use_ext_force=use_ext_force))
...
def simulate_policy(args):
    ...
    if args.residual:
        ...
        env = RobotEnv(show_viewer = True, obs_type = args.obs_type, hole_ori = args.case, testing = True, window_size = args.window_size, ECGIC = args.ECGIC, TCGIC = args.TCGIC, use_ext_force=args.use_ext_force)
    print("Policy loaded")
    if args.gpu:
        set_gpu_mode(True)
        policy.cuda()
